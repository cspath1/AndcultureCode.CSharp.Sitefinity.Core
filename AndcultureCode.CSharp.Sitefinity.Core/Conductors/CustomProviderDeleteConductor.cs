...
namespace AndcultureCode.CSharp.Sitefinity.Core.Conductors
{
    public class CustomProviderDeleteConductor : CustomProviderConductorBase, ICustomProviderDeleteConductor
    {
        ...
        public IResult<bool> Restore<T>(Guid id) where T : DataItemEntity => getProvider().Restore<T>(id);

        public IResult<bool> Restore<T>(T entity) where T : DataItemEntity => getProvider().Restore(entity);
    }
}
